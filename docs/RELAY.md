...
| 겹 | 무엇을 막나 | 구현 | 한계(정직) |
|---|---|---|---|
| ① 등록 속도 | 이름 대량 선점 | IP 당 시간당 **30건** · 전체 시간당 **200건**(고정창) | IP 는 바꿀 수 있다 · ⚠아래 NAT 각주 |
...
⚠**【결정필요】 NAT 각주 — 워크숍에서 정상 참가자가 막힌다**: 등록 상한은 **IP 당**이다.
  같은 장소에서 여러 사람이 한 회선(공유기·기관 NAT) 뒤에 있으면 **한 IP 로 보인다.**
  9/14 워크숍처럼 한자리에서 다 같이 설치하면 31번째 사람부터 한 시간 동안 등록이 막힌다.
  · 지금 값(30/시간)은 **소규모 모임까지만** 버틴다.
  · 선택지 ⓐ상한을 더 올린다(남용 창이 넓어진다) ⓑ워크숍 당일만 임시로 올린다(운영 부담)
    ⓒIP 축을 빼고 **소유 증명 + 전체 상한**만 쓴다(키 생성 비용이 유일한 마찰이 된다).
  · 내 권고 = **ⓒ**. 소유 증명이 이미 필수라 「키를 만들어야 한다」가 자연 마찰이고,
    IP 축은 **정상 사용자를 막는 쪽으로만** 강하게 작동한다. 다만 이것은 운영 판단이라 master 게이트로 올린다.
