...
★**아직 아무도 이 함수를 부르지 않는다**(S5-4 시점). 호출 자리는 `watch` 인데 그 CLI 는
  ...
  그리고 붙일 때 **정해야 하는 것**이 하나 있다: 대조를 **매 주기 도는가, 더 뜸하게 도는가**.
  삭제 검사는 스레드의 전 페이지를 읽으므로 watch 가 아끼려던 바로 그 비용을 다시 쓴다
  (S4-2 실측: 매 주기 전 스레드 읽기는 참가자 한 명으로도 시간당 한도를 넘는다).
  ⇒ 비용이 걸린 운영 정책이므로 **구현자가 임의로 정하지 않는다.**
"""
