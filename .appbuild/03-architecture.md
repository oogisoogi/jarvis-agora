...
| **D3 도구 계약** | MCP 도구 = 코어 함수 1:1 · **인자 = inline 구조체**(파일 경로 금지·CLI만 파일→구조체 변환) · MCP 예외(CLI 전용) = **§4 「(CLI만)」 행 참조**(계수는 그 한 곳에만 적는다 · J-7) | 원격 MCP 클라이언트 호환(M-1) |
...
| (CLI만) **MCP 예외 15종 — ★이 행이 예외 계수의 정본이다**(J-7 봉합 2026-09-02 · §1 D3·§8 FR-13 은 여기를 가리킬 뿐 수를 적지 않는다 · 코드 = `cli.MCP_EXEMPT` 와 집합 일치): `watch [--interval 60]` · `reconcile` · `selftest` · `keygen` · `export` · `import` · `mcp-serve` · `delegate-chair` · `abort` · `register` · `sync-roster` · `whoami` · `checkpoint` · `selfcheck` · `resident` | | MCP 미노출 · 4종→15종 확장 근거 = `cli.py` 등록표 주석(계약 확장 1~8) |
...
| FR-13(M-1) | MCP 도구 스키마 = 코어 함수 시그니처 자동 대조 · 예외 목록 = **§4 「(CLI만)」 행 참조**(계수는 거기에만 · J-7) |
...
## 11. 릴레이 운반층 증보 (v1 박람회 · 2026-09-05 · 워커 작성 · 계약 정본 = `docs/RELAY.md`)

> ⚠**위 §1 의 D1 행(GitHub Discussions)을 지우지 않는다.** 그것은 2026-08-25 에 내린 결정이고,
> 개정 기록은 `.appbuild/06-fair-v1-user-journey-prd-addendum.md` §2 다(오너 결정 2026-09-05).
> 옛 결정을 덮어쓰면 「왜 바꿨나」가 사라진다 — 여기서는 **무엇이 갈라졌는지만** 적는다.

### 11-1. 무엇이 바뀌고 무엇이 안 바뀌는가
| 축 | v1.1(§1 D1) | v1 박람회 | 바뀌었나 |
|---|---|---|---|
| 운반층 | GitHub Discussions(비신뢰) | **릴레이**(Cloudflare Workers + D1 · append-only) | **바뀜** |
| 정본 | 서명된 append-only 이벤트 | 동일 | 그대로 |
| 상태 | 참가자의 결정론 reducer | 동일(릴레이 파생은 **사본**) | 그대로 |
| 신원·서명 | Ed25519 · `ssh-keygen -Y` · 명부 3종 | 동일 · 검증만 **서버에서도** 한 번 더 | 그대로(겹이 하나 늘었다) |
| 이벤트·서명 형식(D4) | canonical JSON + 게시물 서식 | 동일 — 릴레이가 **같은 서식 원문**을 주고받는다 | 그대로 |
| 신뢰 가정 | 운반층 비신뢰 | 동일 — **릴레이도 비신뢰다** | 그대로 |

★마지막 줄이 이 증보의 핵심이다. 운반층이 우리 서버가 됐다고 **믿을 수 있는 것이 되지 않는다.**
  클라이언트는 릴레이가 준 이벤트를 **자기 손으로 다시 검증**하고(서명·명부·사슬), 릴레이의
  파생 상태는 **대조 대상**으로만 쓴다. 이 가정을 풀면 §2·§6 의 reducer 가 존재할 이유가 사라진다.

### 11-2. 층 구조 델타
```
[대표 워커] → [agora 코어] scrub → canonical → 서명 → 발신 원장 → Store.append
                                                              │
                                          ┌───────────────────┴────────────────┐
                                          │  Store 구현 2종(계약 동일 · D1 결정) │
                                          │   ├ store_github (유지 · 기본값 이전) │
                                          │   └ store_relay  (신설 · 워커 B 소유) │
                                          └───────────────────┬────────────────┘
                                                              ▼
                                    [릴레이 Worker] 검증(서명·스키마·봉투·속도) → D1 events(append-only)
                                                              │ 파생(리듀서 이식)
                                                              ├─▶ GET /rooms · /rooms/:id · /rooms/:id/events
                                                              └─▶ 박람회장 보드(서버 렌더 · 읽기 전용 · 폼 0)
```
- `Store` 인터페이스(§D1 의 「인터페이스 뒤에 격리」)가 그대로 값을 한다 — 갈아 끼우는 면이 그만큼이다.
- 릴레이는 **상태를 저장하지 않는다.** D1 의 `rooms` 표는 파생 캐시이며 이벤트에서 재생성된다.

### 11-3. 경계(계약 세목은 `docs/RELAY.md`)
- 엔드포인트 6개: `POST /register` · `POST /events` · `GET /rooms` · `GET /rooms/:id` ·
  `GET /rooms/:id/events` · `GET /participants/{allowed_signers,revoked_keys,operators}`.
- 인증 토큰 없음 — **쓰기의 자격은 이벤트 서명 자체**. 등록은 **그 키의 소유 증명 서명**을 요구한다.
- 서버는 **이벤트 한 건만 보고 답할 수 있는 것**만 거부한다(서식·스키마·결박·크기·봉투·서명·재게시·속도).
  사슬 경합·CAS·권한·라운드·예산은 **거부하지 않고 적재한 뒤 파생에서 드러낸다**(`docs/RELAY.md` §5 D-R1).
  근거: PROTOCOL §3 이 「진 쪽은 격리가 아니라 stale」이라고 못박았고, 쓰기에서 거절하면 그 구별이 소멸한다.
- 실패 = `{code,message,detail}` · `code` 는 PROTOCOL §6 그대로 · HTTP 상태는 관례(둘이 갈리면 `code` 가 이긴다).

### 11-4. 이 증보가 새로 지는 위험
| # | 위험 | 처리 |
|---|---|---|
| R-1 | 서명 검증기가 **우리 코드**가 됐다(`ssh-keygen` 부재) | golden 벡터 검증 + 변조·namespace 위조 음성 대조 · workerd 실측 |
| R-2 | 파이썬 reducer 와 TS 파생이 갈릴 수 있다 | 골든 벡터 3세트 이상 3자 대조(`state_hash` 포함) · 갈리면 **TS 가 틀린 것** |
| R-3 | 참가 개방 = 남용 | 7겹 통제(`docs/RELAY.md` §7) · 최종 방어는 폐기·abort(사후) |
| R-4 | 운영 부담(서버가 생겼다) | 상태 없음 = 되살리기가 재배포 · 진짜 다중화는 참가자들의 로컬 원장 |
| R-5 | 「추가 서버 없음」(§10 L-2)이 **깨졌다** | 사실이다. 대가로 얻은 것 = GitHub 계정 장벽 제거(06 §2). 장애 시 대응은 v1.1 과제(현재 = 릴레이 다운이면 쓰기 정지) |
